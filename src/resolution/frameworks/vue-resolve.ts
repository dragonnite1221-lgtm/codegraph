/**
 * Vue/Nuxt resolver constants, route extraction, and helpers split out of
 * vue.ts to keep it within the 200-line limit. No behavior change.
 */

import { Node } from '../../types';
import { ResolutionContext, FrameworkExtractionResult } from '../types';

export function extractVueRoutes(filePath: string): FrameworkExtractionResult {
    const nodes: Node[] = [];
    const now = Date.now();
 ...
 */
export function filePathToNuxtRoute(normalized: string, afterPagesStart: number): string | null {
